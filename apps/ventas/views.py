...
def cliente_checkout(request):
    """
    Muestra el formulario de checkout y procesa la compra final.
    """
    if request.user.rol != 'Cliente':
        return redirect('usuarios:dashboard')
    ...
    try:
        ...
        messages.error(request, 'No tienes un perfil de cliente asociado. Contacta a soporte.')
        return redirect('usuarios:dashboard')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(request, '¡Compra realizada con éxito! Tu pedido ha sido registrado.')
                return redirect('usuarios:dashboard') 

            except Exception as e:
                print(f"Error al procesar el pedido: {e}") 
                messages.error(request, f'Error al procesar el pedido: {e}')
                return redirect('ventas:cliente_checkout')
...
def crear_pedido_inicial(request):
    """Paso 1: Vendedor crea un Pedido (Borrador) y asigna un Cliente"""
    if request.method == 'POST':
...
def crear_pedido_datos(request, pedido_id):
    ...
    if request.method == 'POST':
        ...
        if not (pedido_form.is_valid() and tipo_form.is_valid()):
            ...
            # (Devolver al render con formularios inválidos)
        else:
            pedido_form.save()
                ...
                if not factura_form.is_valid():
                    ...
                    if modalidad == 'plazos':
                        ...
                        if fecha_venc_user:
                            ...
                            try:
                                doc.fecha_vencimiento = doc.fecha_emision + timedelta(days=int(post.get('dias_plazo')))
...
def agregar_productos_pedido(request, pedido_id):
    ...
    if not hasattr(pedido, 'documentoventa') or not pedido.documentoventa:
        messages.error(request, "⚠️ Este pedido no tiene un documento asociado. Vuelva alPaso 1.")
    ...
    if request.method == 'POST':
        ...
        if 'confirmar_pedido' in request.POST:
            ...
            try:
                with transaction.atomic():
                    pedido.estado = 'Enviado' # O 'Procesando'
                    pedido.save()
                    
                    documento.estado = 'Emitida' # O 'Pagada' si es pago inmediato
                    documento.save()
                    
                    # Detalles del Documento (ya deberían estar creados en paso 2, pero OK)
                    DetalleDocumento.objects.filter(documento=documento).delete() # Limpiar por si acaso
                    for detalle in detalles:
                        DetalleDocumento.objects.create(
                            documento=documento,
                        ...
                        with transaction.atomic():
                            ...
                            if detalle_existente:
                                ...
                                if producto.stock < nueva_cantidad:
                                    mensaje_error = f"⚠️ Stock insuficiente. Ya tiene {detalle_existente.cantidad} en el carrito."
                                else:
                                    detalle_existente.cantidad = nueva_cantidad
                                    detalle_existente.save() # El save() recalcula subtotal
                                    producto.stock -= cantidad # Descontar solo la *nueva* cantidad
                                    producto.save()
...
def actualizar_totales_documento(pedido, documento):
    ...
    pedido.total = total # Sincronizar total del pedido
    pedido.save()


@login_required
def eliminar_producto_carrito(request, pedido_id, producto_id):
    """Elimina un producto del pedido (del vendedor) y restaura el stock"""
    pedido = get_object_or_404(Pedido, id=pedido_id)
    producto = get_object_or_404(Producto, id=producto_id)
    ...
    try:
        with transaction.atomic():
            ...
            if detalle:
                ...
                detalle.delete() # El delete() recalcula el total del pedido
...
def confirmar_pedido(request, pedido_id):
    """(Esta vista parece duplicada con la lógica de 'agregar_productos_pedido', pero la corrijo)"""
    pedido = get_object_or_404(Pedido, id=pedido_id)
    ...
    # ... (Tu lógica de confirmar pedido) ...
    
    # Simulación de la lógica de stock y creación de documento
    try:
        with transaction.atomic():
            # ... (verificar stock, descontar, crear documento, etc.) ...
            pedido.estado = 'Procesando'
            pedido.save()
            messages.success(request, f'El Pedido #{pedido.id} ha sido confirmado.')
